...
describe('samlp (unit tests)', function () {
  ...
  it('should validate a response with encoded CR entities', function(done){
    ...
    var samlXml = fs.readFileSync(__dirname + '/samples/plain/assertion_with_cr.xml').toString();
  ...
  it('should validate a custom response with signed response were the signature has encoded CR entities', function(done){
    // this checks the proper handling of elements where the signature value
    // has an encoded CR entity, e.g.:
    // from https://support.it.auth0.com/inspector/blackbaudinc@us/logs/90020210729154651899611353446428897111878671641083904098
    // <ds:SignatureValue>
    // SpN5TcvO3lz3WBx6McdH1KA+wzuglUqWgkhMhN2ueX31hnjbCDFiv7g6XJUkveyYZQ/QEPhPMZe/&#xd;
    // RlclCvMPO0//T6vNFCAAc37TAW6xe3oQ6aURxU6dBPJj/J8vkO/1aekY6r1ZETa5v1J41P6tbBX6&#xd;
    // qJVN8rbzgfGkm7oqeBYYy5hzEsHjt6kbR1KhECfrKZRDxiNNVRHe8GMUOvmo+kDwANqTfJNehiqL&#xd;
    // 6j4rfeifqoPiUiRNv7hHf35KW1mBvGgozEC35ceMbAFZkM8zoXx9PGHBUKnPyQdttSOjht8KcmB5&#xd;
    // 0UrqSgV6ZDFrZQcH7wRj6uyIH2ucs9NeYpGUPA==
    // </ds:SignatureValue>
    //
    //var samlXml = fs.readFileSync(__dirname + '/samples/plain/qless_sso.xml').toString();
    var samlXml = fs.readFileSync(__dirname + '/samples/plain/signature_values_with_encoded_cr.xml').toString();
    const samlEncoded =  new Buffer(samlXml, 'binary').toString('base64');
    const samlResponse = new Buffer(samlEncoded, 'base64').toString();
    var options = {
      thumbprint: "5CA6E1202EAFC0A63A5B93A43572EB2376FED309",
      ...
      checkDestination: false,
      checkRecipient: false,
      realm: 'urn:auth0:nico-sabena:gss-saml'
    };
    var samlp = new Samlp(options, new Saml(options));
    samlp.validateSamlResponse(samlResponse, function (err, profile) {
      if (err) return done(err);
  ...
  it('should validate a custom response with signed assertion were the signature has encoded CR entities', function(done){
    // this checks the proper handling of elements where the signature value
    // has an encoded CR entity, e.g.:
    // from https://support.it.auth0.com/inspector/blackbaudinc@us/logs/90020210729154651899611353446428897111878671641083904098
    // <ds:SignatureValue>
    ...
    //
    //var samlXml = fs.readFileSync(__dirname + '/samples/plain/qless_sso.xml').toString();
    var samlXml = fs.readFileSync(__dirname + '/samples/plain/signature_values_with_encoded_cr_assertion.xml').toString();
    const samlEncoded =  new Buffer(samlXml, 'binary').toString('base64');
    const samlResponse = new Buffer(samlEncoded, 'base64').toString();
    var options = {
      thumbprint: "5CA6E1202EAFC0A63A5B93A43572EB2376FED309",
