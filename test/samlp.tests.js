...
describe('samlp (unit tests)', function () {
  describe('extractAssertion', function () {
    ...
    it('should returns assertion', function (done) {
      ...
      samlp.extractAssertion(samlpResponse, function (err, assertion) {
        ...
        var doc = new xmldom.DOMParser().parseFromString(assertion.toString(), 'text/xml');
    ...
    it('should return error for invalid XML - "<doc><![CDATA[</doc>"', function (done) {
      const currentSamlResponse = '<doc><![CDATA[</doc>';
      var samlp = new Samlp({});
      try {
        samlp.extractAssertion(currentSamlResponse, function (err, assertion) {
          done(new Error('not expected to finish'));
        });
      } catch (err) {
        // yes this is bunk, to keep backwards compatibility, exceptions are thrown
        // up the stack.
        expect(err).to.exist;
        expect(err).to.have.a.property('message', 'SAMLResponse should be a valid xml');
        done();
      }
    });
  ...
  describe('validateSamlResponse', function(){
    ...
    var samlpResponseWithStatusRequesterWithMessage = '<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" ID="id" InResponseTo="response" Version="2.0" IssueInstant="2014-02-25T15:20:20Z" Destination="https://auth0-dev-ed.my.salesforce.com"><saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">urn:fixture-test</saml:Issuer><samlp:Status><samlp:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Requester"><samlp:StatusMessage>signature required</samlp:StatusMessage></samlp:StatusCode></samlp:Status></samlp:Response>';
    ...
    it(`should handle invalid xml: "<doc><![CDATA[</doc>"`, function(done) {
      var cert = fs.readFileSync(__dirname + '/test-auth0.cer');
      const samlResponse = "<doc><![CDATA[</doc>";
      var options = {
        cert: cert,
        thumbprint: '5CA6E1202EAFC0A63A5B93A43572EB2376FED309',
        checkExpiration: false,
  ...
  it('should reject signature wrapped response', function(done) {
    ...
    var xmlWrapped = outer.replace(/<saml:SubjectConfirmationData[^>]*\/>/, "<saml:SubjectConfirmationData>" + stripped.replace('<?xml version="1.0"?>', "") + "</saml:SubjectConfirmationData>");
    ...
    samlp.validateSamlResponse(xmlWrapped, {}, function (err, profile) {
      expect(err).to.be.ok;
      expect(err.name).to.equals('Error');
      expect(err.message).to.equal('Signature check errors: invalid signature: for uri #pfx2ba35038-7fff-f9c0-c9bc-1462e1455a76 calculated digest is 7fWMoPd8cTH1ssthiMSR6rXLHa4= but the xml to validate supplies digest wFK//X7GAw5PBQHntPWb8OThZEE=');
      done();
    });
  });
  ...
  describe('getSamlRequestParams', function(){
    ...
    describe('signing', function () {
      describe('HTTP-POST or HTTP-Redirect without deflate encoding', function () {
        ...
        // invalid xml that causes an error
        it(`should handle invalid XML - "<doc><![CDATA[</doc>"`, function (done) {
          var samlp = new Samlp({
            identityProviderUrl: server.identityProviderUrl,
            requestTemplate: "<doc><![CDATA[</doc>",
            protocol: 'samlp',
            deflate: false,
            providerName: 'Some name',
            signingKey: {
          ...
          samlp.getSamlRequestParams({}, function(err, _params) {
            expect(err.message).to.equal('fail to compute signature')
            done();
          });
        });
        ...
        [
          '<samlp:AuthnRequest AssertionConsumerServiceURL="https://dev.qld-gov-dev.auth0.com/login/callback?connection=CIDM-AAL2" Destination="https://uat.identity.qld.gov.au:443/authentication/SSOPOST/metaAlias/idp-07-2017" ForceAuthn="false" ID="@@ID@@" IsPassive="false" IssueInstant="@@IssueInstant@@" ProtocolBinding="@@ProtocolBinding@@" ProviderName="@@ProviderName@@" Version="2.0" xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"><!CDATA[<saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">@@Issuer@@</saml:Issuer><samlp:RequestedAuthnContext Comparison="minimum"><saml:AuthnContextClassRef xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">urn:au:qld:gov:authn:names:SAML:2.0:ac:AAL2</saml:AuthnContextClassRef><saml:AuthnContextClassRef xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">urn:au:qld:gov:authn:names:SAML:2.0:attributes:FamilyName,MiddleName,GivenName,email,DateOfBirth</saml:AuthnContextClassRef></samlp:RequestedAuthnContext></samlp:AuthnRequest>',
          '<!CDATA[<samlp:AuthnRequest AssertionConsumerServiceURL="https://dev.qld-gov-dev.auth0.com/login/callback?connection=CIDM-AAL2" Destination="https://uat.identity.qld.gov.au:443/authentication/SSOPOST/metaAlias/idp-07-2017" ForceAuthn="false" ID="@@ID@@" IsPassive="false" IssueInstant="@@IssueInstant@@" ProtocolBinding="@@ProtocolBinding@@" ProviderName="@@ProviderName@@" Version="2.0" xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"><saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">@@Issuer@@</saml:Issuer><samlp:RequestedAuthnContext Comparison="minimum"><saml:AuthnContextClassRef xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">urn:au:qld:gov:authn:names:SAML:2.0:ac:AAL2</saml:AuthnContextClassRef><saml:AuthnContextClassRef xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">urn:au:qld:gov:authn:names:SAML:2.0:attributes:FamilyName,MiddleName,GivenName,email,DateOfBirth</saml:AuthnContextClassRef></samlp:RequestedAuthnContext></samlp:AuthnRequest>'
        ].forEach(xml => {
          it(`should handle invalid XML - "${xml}"`, function (done) {
            var samlp = new Samlp({
              identityProviderUrl: server.identityProviderUrl,
              requestTemplate: xml,
              protocol: 'samlp',
            ...
            samlp.getSamlRequestParams({}, function(err, params) {
              ...
              expect(params.SAMLRequest).to.be.ok
              done();
            });
          });
        });
  ...
  describe('getSamlStatus', function(){
    ...
    it(`should handle invalid XML - "<doc><![CDATA[</doc>"`, function () {
      try {
        this.samlp.getSamlStatus("<doc><![CDATA[</doc>");
        throw new Error('expected an error');
      } catch (error) {
        expect(error).to.be.an.instanceOf(Error);
        expect(error.message).to.equal('SAMLResponse should be a valid xml');
      }
    });
  });
  ...
  describe('decodeResponse', function(){
    it('should decode the SAML response using default settings', function() {
      ...
      const doc = new xmldom.DOMParser().parseFromString(response, 'text/xml');
    ...
    it(`should handle invalid XML - "<doc><![CDATA[</doc>"`, function () {
      const SAMLResponse = new Buffer("<doc><![CDATA[</doc>", 'binary').toString('base64')
      const samlp = new Samlp({});
      samlp.decodeResponse({ body: { SAMLResponse: SAMLResponse } });
    });
  });
});