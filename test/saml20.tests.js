...
describe('saml 2.0 assertion', function () {
  ...
  it('should validate the NameQualifier with the Issuer', function (done) {
    ...
    var publicKey = fs.readFileSync(__dirname + '/test-auth0.cer').toString();
    var saml_passport = new SamlPassport({cert: publicKey, realm: 'urn:myapp', checkRecipient: false, checkExpiration: false});
    var profile = saml_passport.validateSamlAssertion(signedAssertion, function(err, profile) {
  ...
  it('should validate the SPNameQualifier with Audience', function (done) {
