...
var Samlp = module.exports = function Samlp (options, saml) {
  ...
  this.options.default_encoding = encodingMappings[this.options.default_encoding] ||  'utf8';
};
...
Samlp.prototype = {
  ...
  decodeResponse: function(req) {
    var decoded = new Buffer(req.body['SAMLResponse'], 'base64').toString(this.options.default_encoding);

    const encoding = utils.getEncoding(decoded);
    if (encoding && encodingMappings[encoding] && encodingMappings[encoding] !== this.options.default_encoding){
