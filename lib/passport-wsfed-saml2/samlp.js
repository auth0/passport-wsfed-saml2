...
Samlp.prototype = {
  ...
  extractAssertion: function(samlpResponse) {
    ...
    if (token && token.prefix && !token.getAttributeNS(saml2Namespace, token.prefix)) {
      token.setAttribute('xmlns:' + token.prefix, token.lookupNamespaceURI(token.prefix));
    }
  ...
  validateSamlResponse: function (samlResponse, callback) {
    ...
    var assertion = self.extractAssertion(samlResponse);
    ...
    if (isResponseSigned) {
      self._saml.validateSignature(samlResponse, { 
        ...
        signaturePath: samlResponseSignaturePath }, function(err) {
          if (err) return callback(err);
          
          if (!isAssertionSigned) return self._saml.parseAssertion(assertion, callback);
