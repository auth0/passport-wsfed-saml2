...
Samlp.prototype = {
  ...
  extractAssertion: function(samlpResponse, callback) {
    var self = this;
    samlpResponse = utils.parseSamlResponse(samlpResponse, this.eventEmitter);

    var saml2Namespace = 'urn:oasis:names:tc:SAML:2.0:assertion';
    var done = function (err, assertion) {
      if (err) { return callback(err); }

      assertion = utils.parseSamlAssertion(assertion, self.eventEmitter);
  ...
  getSamlStatus: function (samlResponse) {
    ...
    samlResponse = utils.parseSamlResponse(samlResponse, this.eventEmitter);
