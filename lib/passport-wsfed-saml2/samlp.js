...
var Samlp = module.exports = function Samlp (options, saml) {
  ...
  if (this.options.encoding && encodingMappings[this.options.encoding]){
    // the encoding is explicitely set, use that always
    this.options.encoding = encodingMappings[this.options.encoding];
    this.options.explicit_encoding = true;
  } else {
...
Samlp.prototype = {
  ...
  decodeResponse: function(req) {
    var decoded = new Buffer(req.body['SAMLResponse'], 'base64').toString(this.options.encoding);

    if (!this.options.explicit_encoding){
      // Check if using utf8 was right
      const response = utils.parseSamlResponse(decoded);
      if (response.firstChild && response.firstChild.tagName == 'xml'){
        ...
        if (match && match.length >= 2){
          ...
          const encoding = encodingMappings[match[1]];
          if (match[1] !== this.options.encoding && encoding){
            decoded = new Buffer(req.body['SAMLResponse'], 'base64').toString(encoding);
          }
        }
      }
    }
