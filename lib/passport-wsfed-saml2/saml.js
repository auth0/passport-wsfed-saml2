...
SAML.prototype.validateSamlResponse = function (samlResponse, callback) {
  ...
  if (self.options.validateResponse && !self.validateSignature(samlResponse, self.options.cert, self.options.thumbprint)) {
...
SAML.prototype.validateSamlAssertion = function (samlAssertionString, callback) {
  ...
  // Verify signature
  if (self.options.validateSignature && !self.validateSignature(samlAssertionString, self.options.cert, self.options.thumbprint)) {
    return callback(new Error('Invalid signature'), null);
  }
