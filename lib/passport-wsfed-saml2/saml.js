...
SAML.prototype.validateNameQualifier = function (samlAssertion, issuer) {
  var nameID = xpath.select("//*[local-name(.)='NameID']", samlAssertion);
  if (!nameID || nameID.length === 0) return true;

  var nameQualifier = nameID[0].getAttribute('NameQualifier');
  
  if (!nameQualifier) return true;
  return nameQualifier === issuer
};
...
SAML.prototype.validateSPNameQualifier = function (samlAssertion, audience) {
  ...
  var spNameQualifier = nameID[0].getAttribute('SPNameQualifier');
  
  if (!spNameQualifier) return true;
  
  return spNameQualifier === audience  
};
...
SAML.prototype.parseAssertion = function(samlAssertion, callback) {
  ...
  if (!self.validateNameQualifier(samlAssertion, issuer)){
  ...
  if (!self.validateSPNameQualifier(samlAssertion, self.options.realm)){
