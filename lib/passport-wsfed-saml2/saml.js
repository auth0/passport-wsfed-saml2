...
SAML.prototype.validateExpiration = function (samlAssertion) {
  var notBefore = new Date(samlAssertion.Conditions['@'].NotBefore);
  notBefore = notBefore.setMinutes(notBefore.getMinutes() - 10); // 10 minutes clock skew
  
  var notOnOrAfter = new Date(samlAssertion.Conditions['@'].NotOnOrAfter);
...
SAML.prototype.validateAudience = function (samlAssertion, realm) {
  var audience = samlAssertion.Conditions.AudienceRestriction.Audience;
...
SAML.prototype.validateResponse = function (samlAssertionString, callback) {
  ...
  parser.parseString(samlAssertionString, function (err, samlAssertion) {
    ...
    if (!self.validateExpiration(samlAssertion)) {
      return callback(new Error('Token has expired.'), null);
    }

    if (!self.validateAudience(samlAssertion, self.options.realm)) {
