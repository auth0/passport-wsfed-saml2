...
SAML.prototype.validateSignature = function (xml, options, callback) {
  ...
  if (!self.extractAndValidateCertExpiration(xml) && self.options.checkCertExpiration) {
    return callback(new Error('The signing certificate is not currently valid.'), null);
  }

  if (options.cert && !self.validateCertExpiration(options.cert) && self.options.checkCertExpiration) {
    return callback(new Error('The signing certificate is not currently valid.'), null);
  }
...
SAML.prototype.extractAndValidateCertExpiration = function (validatedSamlAssertion) {
  ...
  // Use cert embedded in the assertion if available. If not, we will skip and verify the cert provided through options separately
  const cert = certNodes.length > 0 && certNodes[0].textContent;
  
  // if there is no certificate in the assertion nor provided through options we skip
  // the signature validation will fail later
  if (!cert) { return false; }

  return this.validateCertExpiration(cert);
};

SAML.prototype.validateCertExpiration = function (cert) {
